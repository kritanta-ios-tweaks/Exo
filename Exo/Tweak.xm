...
-(void)startUpdating {
    ...
    {
        @try {
            SBWiFiManager *x = [%c(SBWiFiManager) sharedInstance];
            [self update:@{
                @"wifi.enabled": @([x isPowered]),
                @"wifi.network": [x currentNetworkName] ?: @"",
                @"wifi.strength.current": @([x currentNetworkName] ? [x signalStrengthBars] : 0),
                @"wifi.strength.rssi": @([x currentNetworkName] ? [x signalStrengthRSSI] : 0),
            }];
        }
...
- (void)setPowered:(BOOL)powered 
{
    ...
    [[EXOObserver sharedInstance] update:@{
        ...
        @"wifi.strength.current": @(powered ? [self signalStrengthBars] : 0),
        @"wifi.strength.rssi": @(powered ? [self signalStrengthRSSI] : 0),
    }];
}

-(void)_powerStateDidChange {
    // Not called on iOS 13
    %orig;
    [[EXOObserver sharedInstance] update:@{
...
-(void)_reallySetOperatorName:(NSString *)arg1 inSubscriptionContext:(id)arg2 {
    %orig;
    [[EXOObserver sharedInstance] update:@{
        @"mobile.network": arg1 ?: @""
    }];
